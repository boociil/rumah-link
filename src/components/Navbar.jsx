  ...
  return (
    <div className="fixed bottom-10 w-full flex justify-center items-center z-50">
      <div className="grid relative grid-cols-3 opacity-100 bg-gradient-to-tr min-w-96 bg-white max-h-15 shadow-lg px-2 py-1 mx-4 max-w-lg rounded-xl">
        ...
        <div onClick={onTimClick} className={`home cursor-pointer flex flex-col hover:-translate-y-2 -translate-y-1 justify-center hover:text-[#f79039] items-center ${showTim ? `text-[#f79039]` : `text-gray-500`}  transition-all duration-300`}>
          <AiOutlineTeam className="text-2xl " />
          <p className="text-xs ">Tim</p>
        </div>
        <div className={`${showTim ? `absolute` : `hidden`} text-black bg-white rounded-lg -top-10 right-6 px-4 py-2`}>
          asdasda
        </div>
      </div>
    </div>
