...
export default function LinkBox({
  ...
  return (
    <>
      <Edit isOpen={edit} onClose={() => setEdit(false)} onSuccess={() => {}} linkId={editId} />
      <div
        className={`
      ...
      >
        ...
        <div
          ...
          } transition-all cursor-pointer duration-300 flex flex-col justify-center gap-2 h-full mr-1`}
        >
          <h2 className="font-archivo font-bold text-primary text-xl w-fit">{title}</h2>
        ...
        <div
          ...
          } hidden md:flex cursor-pointer ml-auto flex-col justify-between min-w-8 transition-all duration-300`}
        >
          {/* DELETE */}
          <div
  ...
  `}
        >
          <Image src="arrow-white.svg" alt="Edit" width={10} height={10} />
        </div>
      </div>
    </>
  );
